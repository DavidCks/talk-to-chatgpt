...
async function CN_SayOutLoud(text) {
  ...
  if (!text || CN_SPEAKING_DISABLED) {
    if (
      ...
      console.log(
        "Not starting SpeechRecognition because CN_IS_LISTENING is",
        CN_IS_LISTENING
      );
    }
  ...
  for (let index = 0; index < msgs.length - 1; index++) {
    const msg = msgs[index];
    const onend = msg.onend;
    msgs[index].onend = () => {
      onend();
      CN_IS_READING = true;
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      try {
        ...
        alert(
          "Error with ElevenLabs API text-to-speech conversion: " + e.toString()
        );
      }
    }
  };
...
function CN_CheckNewMessages() {
  ...
  if (currentMessageCount > CN_MESSAGE_COUNT) {
    // New message!
    console.log(
      "New message detected! currentMessageCount: " + currentMessageCount
    );
...
function CN_StartSpeechRecognition() {
  ...
  CN_SPEECHREC.onresult = (event) => {
    ...
    if (
      ...
      if (CN_CONVERSATION_SUSPENDED) {
        console.log(
          "Conversation is currently suspended, voice command ignored. Use the pause word to resume conversation."
        );
        return;
      }
      ...
      if (CN_CONVERSATION_SUSPENDED) {
        console.log(
          "You said '" + CN_SAY_THIS_WORD_TO_PAUSE + "' - Conversation resumed"
        );
...
function DC_ensureTtgptSettingsVisible(posX = undefined, posY = undefined) {
  ...
  const ttgptSettingsBounds = {
    x: ttgptSettingsSize.width + posX,
    y: ttgptSettingsSize.height + posY,
  };
  var newTtgptSettingsPos = DC_minPos(
    ttgptSettingsBounds,
    maxPos,
    -ttgptSettingsSize.width,
    -ttgptSettingsSize.height
  );
  newTtgptSettingsPos = DC_maxPos(newTtgptSettingsPos, { x: 32, y: 0 });
  console.log("- Logging from DC_fetchPostitionFromLocalStrage -");
  console.log(
    `Size: {width: ${ttgptSettingsSize.width}, height: ${ttgptSettingsSize.height}}, Pos: {x: ${posX}, y: ${posY}}, maxPos: {x: ${maxPos.x}, y: ${maxPos.y}}`
  );
  console.log("- End logging from DC_fetchPostitionFromLocalStrage -");
  if (ttgptSettingsEl !== undefined) {
  ...
  for (let i = 0; i < eventNames.length; i++) {
    ...
    snapToTextareaBtn.addEventListener(eventName, function () {
      ...
      try {
        ...
        console.error(
          "Couldn't snap TTGPT Settings to the text area. Error was:"
        );
        console.error(error);
      }
    ...
    snapToTopRightBtn.addEventListener(eventName, function () {
      ...
      try {
        ...
        ttgptSettings.style.left =
          window.innerWidth - 8 - ttgptSettingsSize.width + "px";
        DC_setLocalStragePostition(
          window.innerWidth - 8 - ttgptSettingsSize.width,
          16
        );
...
function CN_InitScript() {
  ...
  jQuery("body").append(
    "<div style='" +
      ...
      " padding: 4px 40px; border-bottom: 1px solid grey;'>" + //4px
  ...
  jQuery(document).on("keydown", function (e) {
    ...
    if (e.altKey && e.shiftKey && e.which === 86) {
      ...
      var wantSpeakOff =
        jQuery(".CNToggle[data-cn=speakon]").css("display") == "none";
      if (wantSpeakOff) {
        // Turn off bot voice
...
function CN_OnSettingsIconClick() {
  ...
  voices +=
    "<option value='" + -1 + "' " + SEL + ">" + "Automatic" + "</option>";
  speechSynthesis.getVoices().forEach(function (voice) {
  ...
  xhr.onreadystatechange = function () {
    ...
    if (xhr.readyState === XMLHttpRequest.DONE) {
      ...
      if (typeof result.voices == "undefined") {
        ...
        // Other
        alert(
          "Unexpected response from ElevenLabs API: " + JSON.stringify(result)
        );
        return;
      }
...
async function splitByLanguage(text) {
  const sentences = splitIntoSentences(text);
  let langDetect = getLanguageDetector();
  let config = { ignore: ["und", "som", "tur"], only: [] };
  let lngs = [];
  ...
  let speechObj = [{ lang: "", sentence: "" }];
  for (const s of sentences) {
    let lang = franc(s, config);
    const langIsUndefined = lang === "und" || lang == undefined || lang == null;
    if (langIsUndefined) {
      lang = !(config.only[0] == undefined || config.only[0] == null)
        ? config.only[0]
        : "eng";
    }
    if (s !== "") {
